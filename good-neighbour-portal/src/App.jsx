...
import { Calendar as CalendarIcon, Clock, User, LogOut, Plus, ChevronLeft, ChevronRight, Briefcase, CalendarDays, Trash2, Users, Heart, Coins, Settings, Receipt, MessageSquare, Search, UserMinus, FileText, Wallet, Info, BookOpen, Star, Sun, Cloud, Zap, Moon, TreePine, Sailboat } from 'lucide-react';
...
function AddShiftModal({ isOpen, onClose, selectedDate, employees = [], clients = [], onSave }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (onSave) onSave({ employeeId, clientId, date: selectedDate, startTime, endTime });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50"><h3 className="text-lg font-bold text-slate-800">Assign New Shift</h3><button onClick={onClose} className="text-slate-400 hover:text-slate-600 text-2xl leading-none">&times;</button></div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div><label className="block text-sm font-medium mb-1">Date</label><input type="date" value={selectedDate} readOnly className="w-full px-3 py-2 border rounded-md bg-slate-50 text-slate-500 cursor-not-allowed" /></div>
          <div><label className="block text-sm font-medium mb-1">Employee</label><select value={employeeId} onChange={e=>setEmployeeId(e.target.value)} className="w-full px-3 py-2 border rounded-md">{safeEmps.map(emp=><option key={emp.id} value={emp.id}>{emp.name}</option>)}</select></div>
          <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium mb-1">Start</label><input type="time" value={startTime} onChange={e=>setStartTime(e.target.value)} className="w-full px-3 py-2 border rounded-md" /></div><div><label className="block text-sm font-medium mb-1">End</label><input type="time" value={endTime} onChange={e=>setEndTime(e.target.value)} className="w-full px-3 py-2 border rounded-md" /></div></div>
          <div><label className="block text-sm font-medium mb-1">Client</label><select value={clientId} onChange={e=>setClientId(e.target.value)} className="w-full px-3 py-2 border rounded-md">{safeClients.map(client=><option key={client.id} value={client.id}>{client.name}</option>)}</select></div>
          <button type="submit" className="w-full bg-teal-600 text-white font-medium py-2 rounded-md hover:bg-teal-700">Save Shift</button>
        </form>
      </div>
    </div>
  );
    ...
    switch (activeAdminTab) {
      ...
      default: return (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b bg-slate-50"><h2 className="text-lg font-semibold flex items-center"><CalendarIcon className="h-5 w-5 mr-2 text-teal-600" />Schedule</h2></div>
          <div className="p-8 text-center text-slate-500">
            <button onClick={() => handleDayClick(new Date().getDate() || 1)} className="bg-teal-600 text-white px-4 py-2 rounded-md hover:bg-teal-700 inline-flex items-center"><Plus className="h-5 w-5 mr-2"/> Add New Shift</button>
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
               {shifts.map(s => {
                 const emp = employees.find(e => e.id === s.employeeId);
                 const c = clients.find(c => c.id === s.clientId);
                 return (
                   <div key={s.id} className="border p-4 rounded-lg bg-slate-50 flex justify-between items-center">
                     <div><div className="font-bold">{s.date}</div><div className="text-sm">{s.startTime} - {s.endTime}</div><div className="text-xs text-slate-500 mt-1">{emp?.name} &bull; {c?.name}</div></div>
                     <button onClick={() => onRemoveShift(s.id)} className="text-red-500 hover:bg-red-100 p-2 rounded"><Trash2 className="h-5 w-5"/></button>
                   </div>
                 )
               })}
            </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex space-x-4 border-b border-slate-200 overflow-x-auto scrollbar-hide pb-2">
        {[ {id: 'schedule', icon: CalendarIcon, label: 'Schedule'}, {id: 'employees', icon: Users, label: 'Employees'}, {id: 'clients', icon: Heart, label: 'Clients'}, {id: 'client-funds', icon: Wallet, label: 'Client Funds'}, {id: 'expenses', icon: Receipt, label: 'Reimbursements'}, {id: 'earnings', icon: Coins, label: 'Earnings'}, {id: 'timeoff', icon: CalendarDays, label: 'Time Off'}, {id: 'paystubs', icon: FileText, label: 'Paystubs'}, {id: 'documents', icon: BookOpen, label: 'Documents'}, {id: 'announcements', icon: MessageSquare, label: 'Announcements'}, {id: 'settings', icon: Settings, label: 'Settings'}].map(tab => (
          <button key={tab.id} onClick={() => setActiveAdminTab(tab.id)} className={`px-2 py-1 font-medium whitespace-nowrap flex items-center ${activeAdminTab === tab.id ? 'text-teal-600 border-b-2 border-teal-600' : 'text-slate-500 hover:text-slate-700'}`}><tab.icon className="h-4 w-4 mr-2" /> {tab.label}</button>
        ))}
      </div>
      {renderAdminTab()}
...
export default function App() {
  ...
  const handleSeedData = async () => {
    ...
    } catch (err) { alert("Error seeding data."); }
  };

  const handleLogin = (username, password) => {
    const foundEmp = employees.find(e => e && e.username && String(e.username).toLowerCase() === String(username).toLowerCase() && e.password === password);
    if (foundEmp) {
  ...
  if (!currentUser) return <LoginPage onLogin={handleLogin} isDbReady={Boolean(isDbReady)} hasData={Boolean(employees.length > 0)} onSeedData={handleSeedData} />;
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        {isAdmin && viewMode === 'admin' ? (
          ...
          <EmployeeDashboard 
            ...
            getClientRemainingBalance={() => 0}
            onUpdateProfile={(id, d) => { runMutation('gn_employees', id, 'update', d); setCurrentUser(prev => ({ ...prev, ...d })); }}
          />
        )}
      </main>
