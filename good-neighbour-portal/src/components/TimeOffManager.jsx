...
import { CalendarDays, Activity, Sun, CheckCircle, XCircle, Trash2, Clock, AlertCircle } from 'lucide-react';
...
export default function TimeOffManager({ employees = [], timeOffLogs = [], onApprove, onReject, onRemoveTimeOff }) {
  ...
  // Calculate Balances dynamically based on approved requests
  ...
  const approvedHistory = safeTimeOffLogs.filter(l => l.status === 'approved').sort((a,b) => new Date(b.startDate) - new Date(a.startDate));
                ...
                <div key={req.id} className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm hover:border-amber-300 transition">
                  ...
                  <div className="flex space-x-2 mt-2">
                    <button 
                      ...
                      className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold py-2 rounded transition flex items-center justify-center"
                    >
                      <CheckCircle className="h-4 w-4 mr-1.5" /> Approve
                    </button>
                    <button 
                      onClick={() => onReject && onReject(req.id)}
                      className="flex-1 bg-white border border-slate-300 hover:bg-red-50 hover:text-red-700 hover:border-red-200 text-slate-600 text-sm font-semibold py-2 rounded transition flex items-center justify-center"
                    >
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center">
            ...
            <h2 className="text-lg font-semibold text-slate-800">Approved Time Off Calendar</h2>
          </div>
          <div className="p-4 space-y-2 max-h-[350px] overflow-y-auto bg-slate-50/50">
            {approvedHistory.length === 0 ? (
              <p className="text-sm text-slate-500 text-center py-8">No approved time off on record.</p>
            ) : (
              approvedHistory.map(log => {
                ...
                return (
                  <div key={log.id} className="flex items-center justify-between p-3 rounded-lg border border-slate-200 bg-white transition hover:shadow-sm">
                    ...
                    <button 
                      onClick={() => {
                        if(window.confirm(`Delete this approved record? This will NOT restore their schedule automatically.`)){
                          onRemoveTimeOff && onRemoveTimeOff(log.id)
                        }
                      }}
