...
// IMPORTANT: Ensure you are importing these helper functions from utils
...
function AdminDashboard({ shifts = [], employees = [], clients = [], expenses = [], clientExpenses = [], paystubs = [], timeOffLogs = [], messages = [], documents = [], currentUser, payPeriodStart, setPayPeriodStart, isBonusActive, setIsBonusActive, bonusSettings, setBonusSettings, onAddEmployee, onRemoveEmployee, updateEmployee, onAddClient, onRemoveClient, updateClient, onUpdateExpense, onUpdateClientExpense, onAddPaystub, onRemovePaystub, onAddTimeOffLog, onRemoveTimeOffLog, onAddDocument, onRemoveDocument, onSendMessage, onAddShift, onRemoveShift, onMarkShiftOpen }) {
  ...
  const renderAdminTab = () => {
    switch (activeTab) {
      ...
      case 'documents': 
        return <DocumentManager documents={documents} onAddDocument={onAddDocument} onRemoveDocument={onRemoveDocument} isAdmin={true} />;
      case 'announcements': 
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        {isAdmin && viewMode === 'admin' ? (
          <AdminDashboard 
            ...
            setPayPeriodStart={(v) => { setPayPeriodStart(v); runMutation('gn_settings', 'global', 'set', { payPeriodStart: v, isBonusActive, bonusAmounts: bonusSettings }); }}
            setIsBonusActive={(v) => { setIsBonusActive(v); runMutation('gn_settings', 'global', 'set', { payPeriodStart, isBonusActive: v, bonusAmounts: bonusSettings }); }}
            setBonusSettings={(v) => { setBonusSettings(v); runMutation('gn_settings', 'global', 'set', { payPeriodStart, isBonusActive, bonusAmounts: v }); }}
