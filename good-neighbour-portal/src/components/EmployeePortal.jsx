    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
      ...
      <div className="p-6 border-b border-slate-200 bg-slate-50/50">
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="bg-amber-50 border border-amber-100 rounded p-2 text-amber-800 text-[10px] font-medium leading-tight">* Keep travel within 15km (max approx $10). Mileage is only covered when traveling <strong>with</strong> the client.</div>
      ...
      <div className="flex-1 p-4 overflow-y-auto max-h-[300px] space-y-2">
        {safeExpenses.length === 0 ? <div className="text-center text-sm text-slate-500 py-4">No mileage logged yet.</div> :
          safeSortByDateDesc(safeExpenses).map(exp => {
            if(!exp) return null; const d = parseLocalSafe(exp.date); const dateStr = isNaN(d.getTime()) ? 'Unknown Date' : d.toLocaleDateString();
            return (
              <div key={exp.id || `exp_${Math.random()}`} className="flex justify-between items-center p-3 border border-slate-100 rounded-lg bg-slate-50">
                <div><div className="font-semibold text-sm text-slate-800">{dateStr}</div><div className="text-xs text-slate-500 mt-0.5">{String(exp.kilometers || 0)} km &bull; {String(exp.description || '')}</div></div>
...
export function EmployeeClientExpenseLog({ myClientExpenses = [], onAddClientExpense }) {
  const [date, setDate] = useState(''); const [amount, setAmount] = useState(''); const [description, setDescription] = useState(''); const [receiptFile, setReceiptFile] = useState(null);
  const safeClientExpenses = Array.isArray(myClientExpenses) ? myClientExpenses : [];
  const handleSubmit = (e) => { e.preventDefault(); if (!date || !amount) return; if(onAddClientExpense) { onAddClientExpense({ date, clientId: 'general', amount: Number(amount), description, receiptDetails: receiptFile ? receiptFile.name : '' }); } setDate(''); setAmount(''); setDescription(''); setReceiptFile(null); };
          ...
          safeSortByDateDesc(safeClientExpenses).map(exp => {
            ...
            return (
              <div key={exp.id || `ce_${Math.random()}`} className="flex justify-between items-center p-3 border border-slate-100 rounded-lg bg-slate-50">
                <div><div className="font-semibold text-sm text-slate-800">{dateStr}</div><div className="text-xs text-slate-500 mt-0.5">${Number(exp.amount || 0).toFixed(2)} &bull; {String(exp.description || '')}</div></div>
                <span className={`text-xs font-bold px-2 py-1 rounded-full ${exp.status==='approved'?'bg-green-100 text-green-800':exp.status==='rejected'?'bg-red-100 text-red-800':'bg-amber-100 text-amber-800'}`}>{String(exp.status || 'pending')}</span>
              </div>
            )
          })
                  ...
                  {dayShifts.map(shift => {
                    ...
                    return (
                      <div key={shift.id || Math.random()} onClick={() => setSelectedClient(client)} className="text-xs p-1.5 rounded bg-teal-100 text-teal-800 border border-teal-200 cursor-pointer hover:bg-teal-200 transition shadow-sm">
                        <div className="font-semibold truncate flex items-center"><Heart className="h-2.5 w-2.5 mr-1 shrink-0 text-teal-600" />{String(client?.name?.split(' ')[0] || 'Unknown')}</div>
                        <div className="text-[10px] mt-0.5 opacity-90 flex items-center"><Clock className="h-2.5 w-2.5 mr-1 shrink-0" />{String(shift.startTime)}-{String(shift.endTime)}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="p-6">
              {nextShift ? (
                <div className="space-y-4">
                  <div className="flex items-center text-slate-700"><CalendarDays className="h-5 w-5 mr-3 text-slate-400" /><span className="font-medium">{String(nextShift.date)}</span></div>
                  <div className="flex items-center text-slate-700"><Clock className="h-5 w-5 mr-3 text-slate-400" /><span className="font-medium">{String(nextShift.startTime)} - {String(nextShift.endTime)}</span></div>
        ...
        <div className="md:w-2/3 space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-0">
                            ...
                            <div key={shift.id || Math.random()} className="p-4 hover:bg-slate-50 transition flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                              <div className="flex items-start space-x-4">
                                ...
                                <div>
                                  ...
                                  <div className="text-sm text-slate-600 flex items-center mt-1"><Clock className="h-3.5 w-3.5 mr-1.5" /> {String(shift.startTime)} - {String(shift.endTime)}</div>
                                </div>
                              </div>
                      ...
                      openShifts.map(shift => {
                        ...
                        return (
                          <div key={shift.id || Math.random()} className="bg-white border border-amber-200 rounded-lg p-4 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div><div className="font-bold text-slate-800">{String(shift.date)}</div><div className="text-sm text-slate-600 mt-1">{String(shift.startTime)} - {String(shift.endTime)} &bull; {String(client?.name)}</div></div>
              ...
              {activeTab === 'expenses' && (<div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-slate-200"><EmployeeMileageLog myExpenses={myExpenses} onAddExpense={(exp) => onAddExpense({ ...exp, employeeId: currentUser.id })} /><EmployeeClientExpenseLog myClientExpenses={myClientExpenses} onAddClientExpense={(exp) => onAddClientExpense({ ...exp, employeeId: currentUser.id })} /></div>)}
