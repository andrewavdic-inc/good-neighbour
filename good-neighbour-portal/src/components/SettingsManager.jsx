...
import { Settings, Award, CalendarDays, Trophy, Medal, HardDrive, RefreshCw, Info, Server, MapPin, AlertTriangle } from 'lucide-react';
import { getStorage, ref, listAll, getMetadata } from 'firebase/storage';

export default function SettingsManager({ onHardReset, payPeriodStart, setPayPeriodStart, isBonusActive, setIsBonusActive, bonusSettings, setBonusSettings, officeLocation, setOfficeLocation }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        {/* DANGER ZONE CARD */}
        <div className="bg-white rounded-xl shadow-sm border border-red-200 overflow-hidden h-fit">
           ...
           <div className="p-6">
             <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
               <div>
                 <h3 className="font-bold text-slate-800">Factory Reset Database</h3>
                 <p className="text-sm text-slate-500 mt-1">Permanently wipes all live data and restores the original mock data sandbox.</p>
               </div>
               <button
                 onClick={onHardReset}
                 className="shrink-0 px-4 py-2.5 bg-red-100 text-red-700 hover:bg-red-600 hover:text-white border border-red-200 hover:border-red-600 rounded-md text-sm font-bold shadow-sm transition-colors"
               >
                 Initialize Reset
               </button>
             </div>
