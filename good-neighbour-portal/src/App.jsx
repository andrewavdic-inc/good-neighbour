...
// ==========================================
// ADMIN DASHBOARD (RESTORED TABS & SCHEDULE)
// ==========================================
function AdminDashboard({ shifts = [], employees = [], setEmployees, updateEmployee, clients = [], setClients, updateClient, expenses = [], onUpdateExpense, clientExpenses = [], onUpdateClientExpense, paystubs = [], onAddPaystub, onRemovePaystub, timeOffLogs = [], onAddTimeOffLog, onRemoveTimeOffLog, messages = [], onSendMessage, currentUser, payPeriodStart, setPayPeriodStart, onAddShift, onRemoveShift, onMarkShiftOpen, onAddEmployee, onRemoveEmployee, onAddClient, onRemoveClient, isBonusActive, setIsBonusActive, bonusSettings, setBonusSettings }) {
  ...
  const renderAdminTab = () => {
    switch (activeTab) {
      case 'employees': 
        return <EmployeeManager employees={employees} onAddEmployee={onAddEmployee} onRemoveEmployee={onRemoveEmployee} updateEmployee={updateEmployee} currentUser={currentUser} />;
      case 'clients': 
        return <ClientManager clients={clients} onAddClient={onAddClient} onRemoveClient={onRemoveClient} updateClient={updateClient} />;
      case 'client-funds': 
        return <AdminClientFundsManager clients={clients} expenses={expenses} clientExpenses={clientExpenses} employees={employees} />;
      case 'expenses': 
        return <ExpenseManager expenses={expenses} clientExpenses={clientExpenses} employees={employees} clients={clients} onUpdateExpense={onUpdateExpense} onUpdateClientExpense={onUpdateClientExpense} />;
      case 'earnings': 
        return <AdminEarningsManager employees={employees} shifts={shifts} expenses={expenses} clientExpenses={clientExpenses} payPeriodStart={payPeriodStart} isBonusActive={isBonusActive} bonusSettings={bonusSettings} />;
      case 'timeoff': 
        return <TimeOffManager employees={employees} timeOffLogs={timeOffLogs} onAddTimeOff={onAddTimeOffLog} onRemoveTimeOff={onRemoveTimeOffLog} />;
      case 'paystubs': 
        return <PaystubManager paystubs={paystubs} employees={employees} onAddPaystub={onAddPaystub} onRemovePaystub={onRemovePaystub} />;
      case 'announcements': 
        return <div className="max-w-4xl"><Announcements messages={messages} onSendMessage={onSendMessage} currentUser={currentUser} employees={employees} /></div>;
      case 'settings': 
        return <SettingsManager payPeriodStart={payPeriodStart} setPayPeriodStart={setPayPeriodStart} isBonusActive={isBonusActive} setIsBonusActive={setIsBonusActive} bonusSettings={bonusSettings} setBonusSettings={setBonusSettings} />;
      case 'schedule':
      default: 
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex flex-col">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        {isAdmin && viewMode === 'admin' ? (
          <AdminDashboard 
            shifts={shifts} employees={employees} clients={clients} expenses={expenses} clientExpenses={clientExpenses} paystubs={paystubs} timeOffLogs={timeOffLogs} messages={messages} currentUser={currentUser} payPeriodStart={payPeriodStart} isBonusActive={isBonusActive} bonusSettings={bonusSettings}
            onAddEmployee={onAddEmployee} onRemoveEmployee={onRemoveEmployee} updateEmployee={updateEmployee}
            onAddClient={onAddClient} onRemoveClient={onRemoveClient} updateClient={updateClient}
            onUpdateExpense={onUpdateExpense} onUpdateClientExpense={onUpdateClientExpense}
            onAddPaystub={onAddPaystub} onRemovePaystub={onRemovePaystub}
            onAddTimeOffLog={onAddTimeOffLog} onRemoveTimeOffLog={onRemoveTimeOffLog}
            onSendMessage={onSendMessage} onAddShift={onAddShift} onRemoveShift={onRemoveShift} onMarkShiftOpen={onMarkShiftOpen}
          ...
          <EmployeeDashboard 
            shifts={shifts} employees={employees} currentUser={currentUser} clients={clients} expenses={expenses} onAddExpense={onAddExpense} clientExpenses={clientExpenses} onAddClientExpense={onAddClientExpense} paystubs={paystubs} timeOffLogs={timeOffLogs} messages={messages} onSendMessage={onSendMessage} payPeriodStart={payPeriodStart} onPickupShift={onPickupShift} isBonusActive={isBonusActive} bonusSettings={bonusSettings} setSelectedClient={setSelectedClient}
          />
        )}
      </main>
