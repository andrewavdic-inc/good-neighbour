...
function EditEmployeeModal({ employee, onClose, onSave }) {
  ...
  const toggleAvailability = (dayPart) => {
    setFormData(prev => {
      ...
      if (current.includes(dayPart)) return { ...prev, availability: current.filter(d => d !== dayPart) };
            ...
            <div className={activeTab === 'profile' ? 'block space-y-6' : 'hidden'}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  {/* UPDATE: Missing Edit Photo input is now restored! */}
                  <div>
            ...
            <div className={activeTab === 'financial' ? 'block space-y-6' : 'hidden'}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <h4 className="text-sm font-bold text-slate-800 border-b border-slate-200 pb-2 pt-4">Availability</h4>
          ...
          {filteredEmployees.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {filteredEmployees.map(emp => {
                ...
                // Use the safe URL cleaner to prevent broken markdown strings
                ...
                return (
                  <div key={emp.id} className="border border-slate-200 rounded-xl p-5 flex flex-col bg-white hover:border-teal-300 hover:shadow-md transition duration-200 relative group">
                    ...
                    <div className="flex items-center space-x-4 mb-4 pr-16">
                      {finalPhotoUrl ? (
                        <img src={finalPhotoUrl} alt={emp.name} className="h-14 w-14 rounded-full border-2 border-teal-100 object-cover shadow-sm shrink-0" />
                      ) : (
        ...
        <form onSubmit={handleAddEmployee} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" value={newPhone} onChange={(e) => setNewPhone(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm focus:ring-teal-500 focus:border-teal-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Email</label>
              <input type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm focus:ring-teal-500 focus:border-teal-500" />
            </div>
          </div>
